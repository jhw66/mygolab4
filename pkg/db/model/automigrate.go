...
func automigrate(db *gorm.DB) error {
	...
	if err := db.AutoMigrate(&User{}, &Video{}, &Favorite{}, &Comment{}, &CommentFavorite{},
		&Relation{}, &ChatRoom{}, &ChatMember{}, &ChatMessage{}); err != nil {
		return fmt.Errorf("auto migrate failed:%w", err)
	}

	targets := []constraintTarget{
		{model: &Video{}, name: "User"},
		...
		{model: &ChatMember{}, name: "ChatRoom"},
		{model: &ChatMessage{}, name: "ChatRoom"},
	}
	if err := ensureAssociationConstraints(db, targets); err != nil {
		return fmt.Errorf("ensure association constraints failed:%w", err)
	}
	return nil
}

func ensureAssociationConstraints(db *gorm.DB, targets []constraintTarget) error {
	for _, target := range targets {
		if db.Migrator().HasConstraint(target.model, target.name) {
			if err := db.Migrator().DropConstraint(target.model, target.name); err != nil {
				return fmt.Errorf("drop constraint %s failed:%w", target.name, err)
			}
		}
		if err := db.Migrator().CreateConstraint(target.model, target.name); err != nil {
			return fmt.Errorf("create constraint %s failed:%w", target.name, err)
		}
	}
	return nil
}
