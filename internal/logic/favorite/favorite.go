...
func (l *FavoriteLogic) Favorite(req *types.FavoriteReq) (resp *types.CommonRsp, err error) {
	...
	if _, err := l.svcCtx.VideoRepo.FindByID(context.Background(), req.Vid); err != nil {
		return &types.CommonRsp{Status: 404, Msg: "未找到该视频"}, errors.New("未找到该视频")
	}

	var liked bool
	_, err = l.svcCtx.FavoriteRepo.FindByUserAndVideo(l.ctx, user.ID, req.Vid)
	switch {
	case err == nil:
		liked = false
		err = l.svcCtx.FavoriteRepo.DeleteByUserAndVideo(l.ctx, user.ID, req.Vid)
		if err != nil {
